...
There is a drawback using this method is that it categorizes all users behind an ISP router as the same one user. For example, if me and 3 people visited the same article in a period of 10 minutes, and if the revisit threshold was set to 10 minutes, then the code would count this as 1 hit. A more accurate way of counting each user would be to use a tracker on the client side, such as a cookie or HTML5's LocalStorage. However, one can completely evade the system by respectively clearing the cookies and by disabling JavaScript. Thus, a more accurate server-side counting method needs to be explored in the future. 

In order to find out the average DHCP lease time set by routers, I resorted to a few network engineers and contacted 3 prominent ISPs (Internet Service Providers). Here are the results:
\vspace{0.3in}
...
\item Based on the average read speed, calculated the time spent on the news article. If the page was kept open indefinitely by the reader, I simply used the maximum expected read time for that article. While this was not 100\% accurate, it was highly reflective of the level of interest with the visited articles. 
\end{enumerate}     
...
Now that the database was created, I needed a way to retrieve the data from it. This involved creating configuration files, classes that abstracted the tables, and code that performed the queries and outputted the resulting data in JSON.

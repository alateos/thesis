...
\end{enumerate}
Note that both of these files are stored one directory above \textbf{public\_html/} for security reasons. This folder is called \textbf{lib/}. Thus, the two files are not accessible directly to the public and accordingly need to be included in other PHP files. It is important that I bring attention to the constant \textbf{WAIT\_LIMIT}, which is the predefined amount of time that the web app should wait before considering the subsequent visit as a new one. This limit is by default set to 60 seconds. 
